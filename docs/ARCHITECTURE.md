...
- **Enterprise AI gateway:** server-only TXT/PDF/DOCX parsers normalize untrusted documents before a provider-neutral, schema-validated adapter boundary. Approved prompt versions, safe operation metadata, limits, privacy approval, bounded retries, and human review are independent controls; credentials never enter application tables.
