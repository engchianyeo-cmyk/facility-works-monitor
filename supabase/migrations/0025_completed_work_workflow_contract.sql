...
begin
  if current_user <> 'postgres' then raise exception '0025 must be applied as postgres'; end if;
  ...
  then raise exception '0025 prerequisite missing: apply 0024 after 0023 first'; end if;
  ...
  if transition_search_path is null or not ('search_path=pg_catalog'=any(transition_search_path)) then
    raise exception '0025 prerequisite mismatch: transition function security contract is unexpected';
  end if;
end;
$preflight$;
