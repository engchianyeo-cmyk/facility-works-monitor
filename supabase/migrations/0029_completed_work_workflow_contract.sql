-- WP-WO-010: Completed Work workflow contract.
-- Preserves 0017 audit/rework/notification mechanics while implementing the approved
-- Assigned/In Progress -> Completed path, mandatory After evidence, and independent review.
begin;

do $preflight$
declare transition_search_path text[];
begin
  if current_user <> 'postgres' then raise exception '0029 must be applied as postgres'; end if;
  if to_regprocedure('public.transition_work_order(uuid,text,jsonb)') is null
  ...
  then raise exception '0029 prerequisite missing: apply 0028 after 0027 first'; end if;
  select p.proconfig into transition_search_path from pg_proc p join pg_namespace n on n.oid=p.pronamespace
  ...
  if transition_search_path is null or not ('search_path=pg_catalog'=any(transition_search_path)) then
    raise exception '0029 prerequisite mismatch: transition function security contract is unexpected';
  end if;
end;
$preflight$;
