...
-- Apply only after migrations 0012-0027 on the FMWorks Preview/UAT chain.
begin;

do $preflight$
begin
 if current_user <> 'postgres' then raise exception '0028 must be applied as postgres'; end if;
 if to_regclass('public.work_orders') is null or to_regclass('public.profiles') is null or to_regclass('public.vendors') is null or to_regclass('public.evidence_items') is null or to_regprocedure('public.transition_work_order(uuid,text,jsonb)') is null then raise exception '0028 prerequisite missing'; end if;
end;
$preflight$;
...
-- in the next migration before live UAT. 0028 establishes the schema, role, evidence and
