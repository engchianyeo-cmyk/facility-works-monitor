...
-- Release-1 field-evidence ownership contract:
-- Work Order Before/After field evidence is recorded by the responsible Technician only.
-- Management roles may review evidence but are not field-evidence upload actors.
...
begin
  ...
  if not public.pilot_account_ready(actor_id) then
  ...
  if p_parent_type='work_order' then
    ...
    if not exists(
      ...
      where i.id=p_parent_id and (
        ...
        or (
          i.assigned_team_id is not null
          and exists(
            select 1 from public.maintenance_team_members m
            where m.team_id=i.assigned_team_id
  ...
  if p_storage_path not like 'evidence/'||pg_catalog.replace(p_parent_type,'_','-')||'/'||p_parent_id::text||'/%'
    or not exists(
      ...
      where o.bucket_id='field-evidence' and o.name=p_storage_path
    )
  then
  ...
  values(
    actor_id,result.work_order_id,result.incident_id,'evidence_uploaded',actor_name,
...
exception
  ...
  when unique_violation then
    return pg_catalog.jsonb_build_object('ok',false,'code','DUPLICATE_EVIDENCE');
  when others then
    return pg_catalog.jsonb_build_object('ok',false,'code','INTERNAL_ERROR');
end;
$function$;

revoke all on function public.register_evidence_item(text,uuid,text,text,bigint,text,text,text)
  from public,anon,service_role;
grant execute on function public.register_evidence_item(text,uuid,text,text,bigint,text,text,text)
  to authenticated;
...
begin
  select pg_catalog.pg_get_functiondef(p.oid),p.prosecdef,p.proconfig
    into definition,secured,config
  from pg_catalog.pg_proc p
  where p.oid='public.register_evidence_item(text,uuid,text,text,bigint,text,text,text)'::regprocedure;

  if not secured
    or config<>array['search_path=pg_catalog']
  ...
  then
    raise exception '0039 postcondition failed: Technician-only Work Order evidence contract mismatch';
  end if;

  if has_function_privilege('public','public.register_evidence_item(text,uuid,text,text,bigint,text,text,text)','EXECUTE')
    or has_function_privilege('anon','public.register_evidence_item(text,uuid,text,text,bigint,text,text,text)','EXECUTE')
    or has_function_privilege('service_role','public.register_evidence_item(text,uuid,text,text,bigint,text,text,text)','EXECUTE')
    or not has_function_privilege('authenticated','public.register_evidence_item(text,uuid,text,text,bigint,text,text,text)','EXECUTE')
  then
    raise exception '0039 postcondition failed: register_evidence_item execute ACL mismatch';
  end if;
end$$;
