  ...
  return (
    <main className="mx-auto max-w-[1500px] space-y-6 p-4 sm:p-6 lg:p-8">
      <header className="rounded-3xl bg-slate-950 px-6 py-7 text-white sm:px-8">
        ...
        <div className="mt-2 flex flex-wrap items-end justify-between gap-4"><div><h1 className="text-3xl font-black tracking-tight sm:text-4xl">{technician ? `Facility Work · ${data.name}` : "Operations Workspace"}</h1><p className="mt-2 text-sm text-slate-300">{technician ? "All Work Orders in your active Facility membership. Assignment controls execution, not visibility." : "Attention, work ownership and decisions for today’s operation."}</p></div><Link href="/" className="rounded-lg border border-slate-600 px-4 py-2 text-sm font-bold hover:bg-slate-800">← Mission Control</Link></div>
      </header>
      ...
      <nav aria-label="Operations views" className="flex gap-2 overflow-x-auto border-b border-slate-200 pb-2">
        {visibleViews.map((item) => <button key={item.id} type="button" onClick={() => setView(item.id)} aria-pressed={view === item.id} className={`min-h-11 shrink-0 rounded-lg px-4 py-2 text-sm font-black ${view === item.id ? "bg-slate-950 text-white" : "text-slate-600 hover:bg-slate-100"}`}>{technician && item.id === "today" ? "Attention Required" : item.label}</button>)}
      </nav>
