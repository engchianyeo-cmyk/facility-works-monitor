...
export default function WorkOrderActions(props: Props) {
  ...
  const actions = useMemo(
    () => authorizedExecutionActions(props.status, props.allowedActions),
  ...
  async function duplicate() {
    ...
    try {
      ...
      setSubmissionState("online");
      router.push(`/work-orders/${result.data.id}/edit`);
      router.refresh();
    } catch {
      setSubmissionState("failed");
  ...
  const stateLabel = submissionState === "online"
    ? "ONLINE"
      ...
      <div className="space-y-5 p-5 sm:p-6">
        {props.currentRework && (
          <section aria-labelledby="rework-context-title" className="rounded-xl border-2 border-orange-300 bg-orange-50 p-4 text-orange-950">
            ...
            <h3 id="rework-context-title" className="mt-1 font-black">Completed Work rejected for correction</h3>
            ...
            <p className="mt-2 text-xs">Returned by {props.currentRework.actor} on {dateTimeLabel(props.currentRework.returnedAt)}. Prior Completed Work evidence remains in the audit record.</p>
            <p className="mt-2 text-sm font-bold">When resubmitting, labour hours must be the cumulative total across all work cycles.</p>
          </section>
        )}
        ...
        {props.reviewContext && decisionActions.length > 0 && (
          <section aria-labelledby="completion-decision-title" className="space-y-4 rounded-xl border-2 border-violet-200 bg-violet-50 p-4 text-violet-950">
            <div>
              <p className="text-xs font-black uppercase tracking-wide text-violet-700">Completed Work verification</p>
              <h3 id="completion-decision-title" className="mt-1 text-lg font-black">Verify Completed Work</h3>
            </div>
            ...
            {props.reviewContext.priorCycles.length > 0 && (
              <details className="rounded-lg border border-violet-200 bg-white p-3">
                <summary className="cursor-pointer font-bold">Prior rework cycles ({props.reviewContext.priorCycles.length})</summary>
                <ol className="mt-3 space-y-3">
                  {props.reviewContext.priorCycles.map((cycle) => (
                    <li key={`${cycle.cycle}-${cycle.returnedAt}`} className="border-l-4 border-orange-300 pl-3 text-sm">
                      <p className="font-bold">Cycle {cycle.cycle}: {cycle.reason}</p>
                      <p className="mt-1">Prior Completed Work: {cycle.completionNotes ?? "Unavailable"} · {cycle.cumulativeLabourHours ?? "Unavailable"} hours · {cycle.evidenceIds.length} evidence item(s)</p>
                    </li>
                  ))}
                </ol>
              </details>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              {decisionActions.some(({ action }) => action === "review") && (
                <button type="button" disabled={busy !== null} onClick={() => openAction("review")} className="min-h-12 rounded-xl bg-emerald-700 px-5 font-black text-white disabled:opacity-50">Verify Completed Work</button>
              )}
              {decisionActions.some(({ action }) => action === "return_for_rework") && (
                <button type="button" disabled={busy !== null} onClick={() => openAction("return_for_rework")} className="min-h-12 rounded-xl border-2 border-orange-400 bg-white px-5 font-black text-orange-900 disabled:opacity-50">Reject & Reopen</button>
              )}
            </div>
          </section>
        )}
        ...
        {interaction === "review" && (
          <form onSubmit={submitReview} aria-describedby="review-help execution-error" className="space-y-4 rounded-xl border border-emerald-200 bg-emerald-50 p-4">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <button disabled={busy !== null} className="min-h-12 rounded-xl bg-emerald-700 px-5 font-black text-white disabled:opacity-50">{busy === "review" ? "Recording verification…" : "Confirm Verification"}</button>
              <button type="button" disabled={busy !== null} onClick={() => setInteraction(null)} className="min-h-12 rounded-xl border border-slate-300 bg-white px-5 font-bold">Back</button>
            </div>
          </form>
        )}

        {interaction === "return_for_rework" && (
          <form onSubmit={submitRework} aria-describedby="rework-help execution-error" className="space-y-4 rounded-xl border border-orange-300 bg-orange-50 p-4">
            <div>
              <h3 className="font-black text-orange-950">Reject & Reopen</h3>
              <p id="rework-help" className="mt-1 text-sm text-orange-900">State exactly what must be corrected. The prior Completed Work and evidence remain in the immutable activity history.</p>
            </div>
            <label className="block text-sm font-bold text-orange-950">Required correction <span aria-hidden="true">*</span>
              <textarea required rows={4} maxLength={2000} value={reworkReason} onChange={(event) => setReworkReason(event.target.value)} className="mt-1 w-full rounded-lg border border-orange-300 bg-white p-3" />
            </label>
            <div className="grid gap-3 sm:grid-cols-2">
              <button disabled={busy !== null} className="min-h-12 rounded-xl bg-orange-700 px-5 font-black text-white disabled:opacity-50">{busy === "return_for_rework" ? "Reopening…" : "Confirm Reject & Reopen"}</button>
              <button type="button" disabled={busy !== null} onClick={() => setInteraction(null)} className="min-h-12 rounded-xl border border-slate-300 bg-white px-5 font-bold">Back</button>
            </div>
          </form>
        )}
        ...
        {interaction === "approve" && (
          <form onSubmit={submitApproval} aria-describedby="approval-help execution-error" className="space-y-4 rounded-xl border border-blue-200 bg-slate-50 p-4">
            <div><h3 className="font-black">Confirm approval</h3><p id="approval-help" className="mt-1 text-sm text-slate-600">An override reason is required by the server only for an Administrator approving their own request.</p></div>
            <label className="block text-sm font-bold">Override reason, when applicable
              <textarea rows={3} maxLength={1000} value={approvalReason} onChange={(event) => setApprovalReason(event.target.value)} className="mt-1 w-full rounded-lg border border-slate-300 bg-white p-3 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-blue-200" />
            </label>
            <div className="grid gap-3 sm:grid-cols-2">
              <button disabled={busy !== null} className="min-h-12 rounded-xl bg-blue-700 px-5 font-black text-white disabled:opacity-50">{busy === "approve" ? "Submitting approval…" : "Confirm approval"}</button>
              <button type="button" disabled={busy !== null} onClick={() => setInteraction(null)} className="min-h-12 rounded-xl border border-slate-300 bg-white px-5 font-bold">Back</button>
            </div>
          </form>
        )}
        ...
        {interaction === "cancel" && (
          <form onSubmit={submitCancellation} aria-describedby="cancellation-help execution-error" className="space-y-4 rounded-xl border border-red-200 bg-red-50 p-4">
            <div><h3 className="font-black text-red-950">Cancel Work Order</h3><p id="cancellation-help" className="mt-1 text-sm text-red-900">Cancellation is recorded in activity history and does not delete the Work Order.</p></div>
            <label className="block text-sm font-bold text-red-950">Cancellation reason <span aria-hidden="true">*</span>
              <textarea required rows={4} maxLength={2000} value={cancellationReason} onChange={(event) => setCancellationReason(event.target.value)} className="mt-1 w-full rounded-lg border border-red-300 bg-white p-3 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-red-200" />
            </label>
            <div className="grid gap-3 sm:grid-cols-2">
              <button disabled={busy !== null} className="min-h-12 rounded-xl bg-red-700 px-5 font-black text-white disabled:opacity-50">{busy === "cancel" ? "Submitting cancellation…" : "Confirm cancellation"}</button>
              <button type="button" disabled={busy !== null} onClick={() => setInteraction(null)} className="min-h-12 rounded-xl border border-slate-300 bg-white px-5 font-bold">Keep Work Order</button>
            </div>
          </form>
        )}

        {(props.canEdit || props.allowedActions.includes("cancel") || props.canDuplicate) && (
          <div className="border-t border-slate-200 pt-4">
            <p className="mb-3 text-xs font-black uppercase tracking-wide text-slate-500">Secondary actions</p>
            <div className="grid gap-3 sm:grid-cols-2">
              {props.canEdit && <button type="button" disabled={busy !== null} className="min-h-11 rounded-lg border border-slate-300 px-4 text-sm font-bold focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-slate-300" onClick={() => router.push(`/work-orders/${props.id}/edit`)}>Edit Work Order</button>}
              {props.allowedActions.includes("cancel") && <button type="button" disabled={busy !== null} onClick={() => openAction("cancel")} className="min-h-11 rounded-lg border border-red-300 px-4 text-sm font-bold text-red-700 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-red-200">Cancel Work Order</button>}
              {props.canDuplicate && <button type="button" disabled={busy !== null} onClick={() => void duplicate()} className="min-h-11 rounded-lg border border-slate-300 px-4 text-sm font-bold focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-slate-300">{busy === "duplicate" ? "Creating draft…" : "Duplicate as draft"}</button>}
            </div>
          </div>
        )}
