...
test.describe.serial("Pilot identity, password lifecycle and RBAC", () => {
  test("synthetic Administrator can log in and provision a password-pending user", async ({ page }) => {
    ...
    const departmentSelect = provisioning.getByLabel("Department");
    await expect(departmentSelect.locator("option")).toHaveCount(9);
    for (const departmentName of [
      "Administration", "Engineering", "Facility", "Finance", "HR",
      "Operations", "Procurement", "Warehouse",
    ]) {
      await expect(departmentSelect.locator("option", { hasText: departmentName })).toHaveCount(1);
    }
    ...
    await departmentSelect.selectOption({ label: "Facility" });
