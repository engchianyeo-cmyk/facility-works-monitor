...
export async function POST(request: Request, { params }: Context) {
  ...
  try { const { id } = await params; const supabase = await createClient(); const { data, error } = await supabase.rpc("change_asset_status", { p_asset_id: id, p_status: body.status, p_reason: body.reason }); if (error) return assetTransportFailure("change status in"); return assetRpcResponse(data as AssetRpcResult | null); } catch { return assetTransportFailure("change status in"); }
}
