...
export async function PATCH(request: Request, { params }: Context) {
  ...
  try {
    ...
    const { data, error } = await supabase.rpc("update_asset_details", { p_asset_id: id, p_payload: body.details ?? body, p_reason: body.reason ?? null });
    if (error) return assetTransportFailure("update");
    return assetRpcResponse(data as AssetRpcResult | null);
  } catch { return assetTransportFailure("update"); }
}
