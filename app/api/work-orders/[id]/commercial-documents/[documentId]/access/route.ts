...
export async function POST(_: Request, { params }: Context) {
  ...
  const { data, error } = await supabase.from("work_order_commercial_documents").select("storage_path").eq("id", documentId).eq("work_order_id", id).is("deleted_at", null).maybeSingle();
